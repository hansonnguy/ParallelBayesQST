...
%% INPUT
dataFileName = 'simData_Q=4_L=1_P=400';
th = 12;                                     % Thinning value.
fileNum = '001';                            % Saved file name
%% LOOP SETTINGS
dataFile = load(dataFileName,'counts','rhoVecG','Q','P');
counts = dataFile.counts;
rhoVecG = dataFile.rhoVecG;                        % Ground truth.
Q = dataFile.Q;
P = dataFile.P;
clear dataFile;                                    % Clear up memory space.
...
% rhos = zeros(D,D, numSamps, numChains, th + 1, numStates);      % initialize matrix storing rhos (saving all samples)
